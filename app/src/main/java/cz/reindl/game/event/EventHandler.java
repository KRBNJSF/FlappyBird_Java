...
public class EventHandler {
    ...
    public void collision() {
        checkHighScore();
        for (int i = 0; i < view.barriers.size(); i++) {
            if (View.coin.getX() + View.coin.getBitmap().getWidth() < 0 || View.coin.getRect().intersect(view.barriers.get(i).getRect()) || View.coin.getRect().intersect(view.barriers.get(i).getBottomPipeRect())) {
    ...
    private void checkBirdSkin() {
        if (Bird.boughtSkin == 0 && !isRunning && bird.getCoins() >= 1000) {
            Snackbar.make(menuLayout, "A new skin is available for purchase", Snackbar.LENGTH_SHORT).show();
        }
